...
```
对于非正式版发表:
 + 测试版,特点为版号+1接-bete。发布在主页release预发布版，并在mirror内测通道。
    方法：手动运行构建命令 or 提交时在正文最后一行单独写[deploy-beta]。推荐后者，会自动识别。
    进阶：使用预制的交互提交Hook：不写任何提交文段，直接提交，在交互界面根据提示修改，更方便。
 + 构建版，特点为版号接-ci。只要上传到远程仓库就会自动构建，不会再release、mirror出现。
    方法：自动构建，用于验证是否有错误。用户需要手动去构建页面下载。
```
